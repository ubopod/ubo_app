                       ...
                       double value)
{
    ...
    lv_obj_set_size(wrap, lv_pct(100), 78);
...
void ubo_build_home(const ubo_home_view *v)
{
    ...
    lv_obj_set_size(vol, 25, lv_pct(100));
    lv_obj_center(vol);
    lv_bar_set_range(vol, 0, 100);
    lv_bar_set_value(vol, (int)(v->volume_level * 100), LV_ANIM_OFF);
    lv_obj_set_style_radius(vol, 5, LV_PART_MAIN);
    lv_obj_set_style_radius(vol, 5, LV_PART_INDICATOR);
    ...
    lv_obj_set_style_text_font(v_top, ubo_font_icon_18(), 0);
    ...
    lv_obj_set_style_text_font(v_bot, ubo_font_icon_18(), 0);
