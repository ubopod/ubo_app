...
def frame_stream_id(view: object) -> str | None:
    """Return the stream_id when `view` gets its pixels from a frame stream.

    `image_viewer` used to ship the picture inline as a bytes prop, which put a
    multi-megabyte payload on the store stream; it now takes the same path as
    `frame_stream` and carries only width/height in props.
    """
    if isinstance(view, RenderViewData) and view.kind in {
        'frame_stream',
        'image_viewer',
    }:
