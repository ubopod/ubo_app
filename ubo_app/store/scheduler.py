...
from ubo_app.constants import DEBUG_SCHEDULER
...
class Scheduler(threading.Thread):
    ...
    async def call_callback(
        ...
        *,
        ...
        if DEBUG_SCHEDULER:
            await self._call_callback_with_monitoring(callback)
        else:
            try:
                callback()
    ...
    async def _call_callback_with_monitoring(
        self: Scheduler,
        callback: Callable[[], None],
    ) -> None:
        """Call callback with freeze detection and monitoring (DEBUG_SCHEDULER only)."""
        import time
    ...
    async def shutdown(self: Scheduler) -> None:
        ...
        # Log summary on shutdown (only when DEBUG_SCHEDULER is enabled)
        if DEBUG_SCHEDULER and self._start_time > 0:
            import time
