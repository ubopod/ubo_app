...
- Persistent store: `mqtt:broker`, `mqtt:is_enabled`, `mqtt:allow_remote_control`,
  `mqtt:published_components`.
- Secrets (`~/.config/ubo/.secrets.env`): `MQTT_PASSWORD` (external broker),
  `MQTT_BUNDLED_BROKER_PASSWORD` (generated by the docker service for the bundled broker).
- Two broker modes, both reached **outbound** — the pod is never a LAN-facing server:
  - `BUNDLED` — the Mosquitto container in the on-device Home Assistant composition
    (`080-docker/apps/home_assistant.py`), on `127.0.0.1:1883`. The broker runs two listeners:
    an anonymous one confined to `ubo_net` (how HA and peer add-ons reach it as `mosquitto:1883`)
    and an authenticated one published on the host's loopback — `ubo-app` runs on the host, not in
    a container, so the loopback port is its path, and it presents the generated credential from
    the secrets file (`_resolve_target`). Other local processes cannot use the broker without it.
    That publish uses compose's *long* syntax specifically so `apply_compose_port_binding` cannot
    move the broker to `0.0.0.0`.
...
**Off by default.** The bundled broker's `ubo_net` listener is anonymous and reachable by every
container on that network; a LAN broker is only as trustworthy as its own auth. `Home Assistant
Control` is a user control, not an authentication boundary — and MQTT discovery has no pairing
