...
export function Inputs({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!isGrpcConnected) {
      console.error("gRPC not connected");
    ...
    if (input?.fields && input.fields.itemsList.length > 0) {
      ...
      // Fallback: get value directly from the input element if FormData didn't capture it
      if (!value || value.trim() === "") {
        ...
        if (inputElement) {
          ...
          console.log(
            "FormData value was empty, using direct input value:",
            value,
          );
        }
      }
      console.log("Extracted value from field:", firstField.name, "=", value);
    } else {
      value = (formData.get("value") as string) || "";
      // Fallback: get value directly from the input element
      if (!value || value.trim() === "") {
        const inputElement = form.querySelector(
          '[name="value"]',
      ...
      console.log("Extracted value from 'value' field:", value);
    }
    // Get action from formData (submit button value) or fallback to submitter
    const action =
      (formData.get("action") as string) ||
      ((event.nativeEvent as SubmitEvent).submitter as HTMLButtonElement)
        ?.value ||
      "";
    console.log("Form action:", action, "Input ID:", id, "Value:", value);

    if (!action) {
      console.error("No action detected from form submission");
    ...
    if (action === "provide") {
      try {
        ...
        console.log("Dispatching InputProvideAction via gRPC...");
        await store.dispatchAction(dispatchActionRequest);
        console.log("InputProvideAction dispatched successfully");
      } catch (error) {
      ...
      try {
        ...
        console.log("Dispatching InputCancelAction via gRPC...");
        await store.dispatchAction(dispatchActionRequest);
        console.log("InputCancelAction dispatched successfully");
      } catch (error) {
